...
title: 关键词报表
platform: kuaishou
source_url: https://developers.e.kuaishou.com/docs?docType=DSP&documentId=2625&menuId=3349
doc_id: kuaishou_2625
source_id: kuaishou_2625
---
# 关键词报表
...
| version | 0.0.1 |
| bizName | 效果广告 |
| createTime | 2025-08-12T08:01:13.191Z |

## 接口信息

| 字段 | 值 |
| --- | --- |
| targetPath | https://ad.e.kuaishou.com/rest/openapi/gw/dsp/v1/report/word_info_report |
| httpMethod | POST |
...
| 字段 | 类型 | 必填 | 示例 | 说明 | 备注 |
| --- | --- | --- | --- | --- | --- |
| advertiser_id | Long | 是 | 21904898 | 广告主 ID（注：非账户快手 ID），在获取 accessToken 时返回 |  |
| start_date | String |  | 2022-01-29 | 过滤筛选条件，格式 yyyy-MM-dd 可选时间范围参见文档上方说明 | （必选其 2） |
...
| campaign_type | Integer |  | 5 | 计划类型 |  |
| report_dims | String[] |  |  | report_dims |  |
| temporal_granularity | String |  |  | 天粒度（DAILY）／小时粒度（HOURLY），默认支持天粒度数据 | （可选） |
| campaign_ids | Long[] |  | [1234,345] | 计划IDs |  |
| unit_ids | Long[] |  | [123456,234567] | 广告组 ID 集，过滤筛选条件，单次查询数量不超过 5000 | （可选） |
| detailed | Boolean |  | true | 是否按照unit_id分组 |  |
| word_info_ids | Long[] |  | [2345678,567890] | 推广关键词ID集，过滤筛选条件，单次查询数量不超过 5000。推广关键词ID集可通过获取关键词列表接口获取 | （可选） |
| extend_info | String[] |  | - | 自定义列，支持列名及其关联字段详见：https://docs.qingque.cn/d/home/eZQB-fLBIZLvGG50L7vFkHL3J?identityId=1oE314hFZmG | （可选） |
| selected_columns | String[] |  |  | 自定义列 |  |
...
| data | WordInfoReport |  |  | 关键词报表 |  |
| - total_count | Long |  | 40 | 数据的总行数 |  |
| - details | WordInfoReportDetail[] |  | [{}] | 数据明细信息 |  |
| - relevance_score_avg | Double |  | 1.8 | 素材相关性分数 |  |
...
| - event_credit_grant_landing_page | Long |  | 2160 | 落地页授信数 |  |
...
| - word_info_id | Long |  | 81804206 | 推广关键词ID |  |
| - word | String |  | 自定义 | 关键词文本 |  |
| - match_type | Integer |  | 1 | 匹配方式：1:精确匹配,2:短语匹配,3:广泛匹配 |  |
| - campaign_id | Long |  | 25128075 | 广告计划 ID |  |
| - campaign_name | String |  | 【1123铃铛推送】粉丝/直播推广_20211123_16:50 | 广告计划名称 |  |
| - unit_id | Long |  | 3811268 | 广告组 ID |  |
| - unit_name | String |  | C4【极速IOS-H5有】行为数_20211123_17:39_4 | 广告组名称 |  |
        ...
        "details": [
            {
                ...
                "word_info_id": 2800010,
                "word": "拿铁",
                "match_type": 2,
                "campaign_id": 512201555,
                "campaign_name": "搜索广告_提升应用安装_20220209_16:03(勿删)",
                "unit_id": 1001802618,
                "unit_name": "行为数_20220209_16:04（勿删）"
            }
        ]
    },
    "message": "OK",
    "request_id": "5ec40789fee84b429545e5e2a48f183e"
}
```
