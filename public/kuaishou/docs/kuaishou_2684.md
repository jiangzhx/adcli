...
title: 查询定向模板
platform: kuaishou
source_url: https://developers.e.kuaishou.com/docs?docType=DSP&documentId=2684&menuId=3418
doc_id: kuaishou_2684
source_id: kuaishou_2684
---
# 查询定向模板
...
| version | 0.0.4 |
| bizName | 效果广告 |
| createTime | 2026-01-14T03:28:26.009Z |

## 接口信息

| 字段 | 值 |
| --- | --- |
| targetPath | https://ad.e.kuaishou.com/rest/openapi/gw/dsp/target/template/details |
| httpMethod | POST |
...
| 字段 | 类型 | 必填 | 示例 | 说明 | 备注 |
| --- | --- | --- | --- | --- | --- |
| advertiser_id | Long | 是 |  | 广告主id | 广告主 ID，在获取 accessToken 的时候返回 |
| template_id | Long[] |  |  | 模板ID列表 | 默认ID和分页参数二选一必传 |
| page | Integer |  |  | 请求的页码，默认为 1 | page和page_size要么都传，要么都不传；都不传时需要传模板ID |
...
| 字段 | 类型 | 必填 | 示例 | 说明 | 备注 |
| --- | --- | --- | --- | --- | --- |
| data | PageDataListMapiTemplateDetailRespSnake |  |  | 返回数据 |  |
| - current_page | Integer |  |  | 当前页 |  |
| - details | MapiTemplateDetailRespSnake[] |  |  | 详情 |  |
| - create_time | Long |  |  | 定向模板创建时间，"2019-06-11 15:17:25" |  |
| - target | MapiTargetInfo463Snake |  |  | 定向信息 |  |
...
| - behavior_interest | BehaviorInterest883Snake |  |  | 行为兴趣定向 |  |
| - behavior | Behavior822Snake940 |  |  | 行为定向 | behavior 与 interest 同时不传，则清空行为兴趣定向 |
...
| - interest | Interest994Snake293 |  |  | 兴趣定向 | behavior 与 interest 同时不传，则清空行为兴趣定向 |
| - keyword | Keyword[] |  |  | 行为定向关键词 | 根据/rest/openapi/v1/tool/keyword/query 接口获取 |
| - id | Long |  |  | 关键词 id | id 与 name 需互相匹配 |
| - name | String |  |  | 关键词名称 | id 与 name 需互相匹配 |
...
| - celebrity | MapiCelebrity964Snake |  |  | 快手网红 | behaviors、fans_star两个字段必填充 |
| - behaviors | String[] |  |  | 行为类型 | 该字段为平台对应快手网红功能中的行为类型，可多选，但不可不选，不选会导致快手网红定向失效，定义如下 0:关注、1:视频互动、2:直播互动 |
| - fans_stars | CelebrityFansStar294Snake776[] |  |  | 网红内容 | 该字段包括平台对应快手网红功能中的网红分类和快手网红两项，数据保证录入顺序，可从【快手网红-网红分类】和【快手网红-搜索快手网红】接口获取对应数据 |
...
| - device_price | String[] |  |  | 设备价格， | 1：1,500 元以下（近期下线）， 新增：11：1000以下，12：1001~1500，请直接使用新枚举值； 2：1,501~2,000； 3：2,001~2,500；4：2,501~3,000；5、3,001~3,500； 6：3,501~4,000；7：4,001~4,500； 8：4,501~5,000；9：5,001~5,500； 10：5,500 元以上；传值为 [] 表示不限； |
| - disable_installed_app_switch | Integer |  |  | 过滤已安装人群维度 | 白名单可用，0：过滤（默认），1：不限 |
...
| - filter_time_range | Integer |  |  | 过滤已转化人群时间范围 | 0：30天（默认）；1：60天；2：90天； |
| - gender | Integer |  |  | 性别 | 1：女性, 2：男性，0 表示不限 |
| - intelli_extend_option | Integer |  |  | 智能定向开关 | 0：表示自定义定向（默认）；1：智能定向；2：智能定向Pro（白名单可用）；和auto_population字段互斥，只能有一个字段开启 |
...
| - auto_population | Integer |  |  | 智能人群包 | 白名单可用，0：关闭（默认），1：开启；和 intelli_extend_option 字段互斥，只能有一个字段开启 |
| - behavior_interest_param_show | BehaviorInterestParamShowSnake |  |  | 行为兴趣（升级版） | 白名单可用 |
...
| - age_v2 | IntegerRange572504 |  | - | 自定义年龄段 | 年龄区间min，年龄最大限制max |
| - max | Integer |  |  | 年龄最小值 |  |
| - min | Integer |  |  | 年龄最大值 |  |
...
| - template_id | Long |  |  | 定向模板 ID |  |
| - template_name | String |  |  | 定向模板名称 |  |
| - unit_count | Integer |  |  | 绑定 unit 个数 |  |
| - update_time | Long |  |  | 定向模板最近修改时间 |  |
...
```
{
    ...
    "data": {
        ...
        "details": [
            {
                "update_time": 1676537337112,
                "template_name": "测试0088",
                "create_time": 1676537337112,
                "target": {
                    "celebrity": {
                        "fans_stars": [
                            {
    ...
    "message": "OK",
    "request_id": "EMyAgICw0JiOrAEY4gIg_cab5-YwKNeF2dgP"
}
```
...
```
查询定向模板 /rest/openapi/gw/dsp/target/template/details { "advertiser_id": 123123, "template_id": [23232, 9283923], "page": 1, "page_size": 10 } { "code": 0, "data": { "total_count": 282, "details": [ { "update_time": 1676537337112, "template_name": "测试0088", "create_time": 1676537337112, "target": { "celebrity": { "fans_stars": [ { "name": "生活-亚洲生活", "id": "26-228", "category": null, "type": 1 }, { "name": "生活-欧洲生活", "id": "26-118", "category": null, "type": 1 } ], "behaviors": [ 1 ] }, "gender": 2, "media": [], "network": 0, "device_price": [ 5, 6, 7 ], "media_source_type": 0, "user_type": 0, "exclude_population": [ 950011889 ], "device_brand_ids": [], "ages_range": [ 31, 41, 50 ], "android_osv": 3, "disable_installed_app_switch": 0, "behavior_type": 1, "filter_converted_level": 0, "filter_time_range": 0, "platform_os": 1, "population": [ 950012677 ], "behavior_interest": { "interest": { "strength_type": 1, "label": [ "1-101", "1-103", "1-104" ], "keyword": [] }, "behavior": { "strength_type": 1, "time_type": 4, "label": [ "1-101" ], "keyword": [], "scene_type": [ 1, 2, 3, 4 ] } }, "seed_population": null, "ios_osv": null, "app_interest_ids": [], "paid_audience": [], "intelli_extend_option": 0, "region": [ 110101 ], "exclude_media": [] }, "template_id": 4803, "unit_count": 0 } ], "current_page": 1, "page_size": 10 }, "message": "OK", "request_id": "EMyAgICw0JiOrAEY4gIg_cab5-YwKNeF2dgP" } advertiser_id Long 广告主id template_id Long[] 模板ID列表 page Integer 请求的页码，默认为 1 page_size Integer 每页行数 data PageDataListMapiTemplateDetailRespSnake 返回数据 current_page Integer 当前页 details MapiTemplateDetailRespSnake[] 详情 create_time Long 定向模板创建时间，"2019-06-11 15:17:25" target MapiTargetInfo463Snake 定向信息 distance_show DistanceShowSnake[] 新商圈 address String address lng String lng lat String lat radius Long radius location_name String location_name poi_id String poi_id android_osv Integer Android 版本 app_ids String[] APP 行为-按 APP 名称 app_interest_ids String[] APP 行为-按分类 app_names String[] 请补充描述详情 behavior_interest BehaviorInterest883Snake 行为兴趣定向 behavior Behavior822Snake940 行为定向 keyword Keyword[] 行为定向关键词 id Long 关键词 id name String 关键词名称 label String[] 行为定向 类目词 scene_type String[] 行为场景 strength_type Integer 行为强度 time_type Integer 在多少天内发生行为的用户 interest Interest994Snake293 兴趣定向 keyword Keyword[] 行为定向关键词 id Long 关键词 id name String 关键词名称 label String[] 兴趣定向类目词 strength_type Integer 兴趣标签强度 behavior_type Integer 行为兴趣类型 business_interest String[] 请补充描述详情 celebrity MapiCelebrity964Snake 快手网红 behaviors String[] 行为类型 fans_stars CelebrityFansStar294Snake776[] 网红内容 category String[] - id String - name String - type Integer - device_brand_ids String[] 请补充描述详情 device_price String[] 设备价格， disable_installed_app_switch Integer 过滤已安装人群维度 district_ids String[] 商圈定向 exclude_media String[] 媒体定向排除包 exclude_population String[] 排除人群包 filter_converted_level Integer 过滤已转化人群纬度 filter_time_range Integer 过滤已转化人群时间范围 gender Integer 性别 intelli_extend_option Integer 智能定向开关 ios_osv Integer iOS 版本 media String[] 媒体定向包 media_source_type Integer 媒体包来源 network Integer 网络环境 paid_audience String[] 付费人群包 platform_os Integer 定向的 os 版本 population String[] 定向人群包 region String[] 地域 seed_population String[] 种子人群包 target_source Integer 请补充描述详情 user_type Integer 用户类型 ip_type Integer 地域IP auto_population Integer 智能人群包 behavior_interest_param_show BehaviorInterestParamShowSnake 行为兴趣（升级版） category_ids String[] 类目ID keyword_ids Integer[] 关键词ID custom_behavior Integer 自定义行为意向 scene_types Integer[] 场景类型 time_type Integer 时间范围 filter_converted_wechat_id String[] 过滤已转化-企微号ID operators Integer[] 运营商 harmony_osv Integer 鸿蒙操作系统版本号限制 age_v2 IntegerRange572504 自定义年龄段 max Integer 年龄最小值 min Integer 年龄最大值 ages_range_v2 String[] 固定年龄段 shared_user Integer 过滤共用手机用户 template_id Long 定向模板 ID template_name String 定向模板名称 unit_count Integer 绑定 unit 个数 update_time Long 定向模板最近修改时间 page_size Integer 页面大小 total_count Long 总数 message String 返回信息 code Integer 返回码
```
