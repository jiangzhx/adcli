...
| daily_budget* | integer | 投放账户日预算，单位为分，设置为 0 表示不设预算（即不限）日预算需介于 5,000 分-4,000,000,000 分之间（50 元-40,000,000 元，单位为人民币）；修改后的日预算不能低于营销帐号今日已消耗金额加上 5,000 分（50 元，单位为人民币）；每次修改幅度不能低于 5,000 分（50 元，单位为人民币）；允许从不限预算（0）修改为指定预算，以及从指定日预算修改为不限预算（0） |
