...
source_url: https://developers.e.qq.com/v3.0/docs/api/finder_ad_object_list/get
doc_id: tencent_ads_v3_0_docs_api_finder_ad_object_list_get
source_id: tencent_ads_v3_0_docs_api_finder_ad_object_list_get
---
# [全部接口](https://developers.e.qq.com/v3.0/docs/apilist)

| 所属权限 | ads_management |
| --- | --- |
| 请求地址 | finder_ad_object_list/get |
| 请求方法 | GET |
...
| field* | string | 过滤字段 可选值：{ created_time, dynamic_creative_id, ad_export_id } |
| operator* | enum | 操作符，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_filter_operator) 当 field 取值 created_time 时，可选值：{ GREATER_EQUALS, LESS_EQUALS } 当 field 取值 dynamic_creative_id 时，可选值：{ EQUALS, IN } 当 field 取值 ad_export_id 时，可选值：{ EQUALS, IN } |
| values* | string[] | 字段取值，values 数组元素的个数限制与 operator 的取值相关 当 field 取值 created_time 时，数组长度为 1 字段长度为 10 字节 当 field 取值 dynamic_creative_id 时，数组最小长度 1，最大长度 100 字段长度最小 1 字节，长度最大 1024 字节 当 field 取值 ad_export_id 时，数组最小长度 1，最大长度 100 字段长度最小 1 字节，长度最大 1024 字节 |
...
```
curl -v -G 'https://api.e.qq.com/v3.0/finder_ad_object_list/get?access_token=<ACCESS_TOKEN>&timestamp=<TIMESTAMP>&nonce=<NONCE>' \
...
| finder_object | struct | 视频号动态 |
| finder_username | string | 视频号账号 id 已废弃 |
| description | string | 视频号标题 |
| medias | struct | 视频号动态视频 |
| thumb_url | string | 视频号视频首帧图 |
| cover_url | string | 视频号视频封面图 |
| url | string | 视频号视频 |
| description | string | 视频号标题 |
| video_play_len | integer | 视频播放时长，单位：秒 |
| flag | integer | 动态标记类型 |
| wechat_channels_account_id | string | 视频号账号 id |
| wechat_channels_account_name | string | 视频号名称 |
| bullet_comment_switch_open | boolean | 是否打开视频号弹幕开关，true：是，false：否 |
| bullet_comment_supported | boolean | 是否支持视频号弹幕，true：是，false：否 |
| object_private_switch_open | boolean | 是否打开视频号动态设置为私密开关，true：是，false：否 |
| object_private_disable_code | enum | 不支持视频号动态设置为私密的状态码，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#object_private_disable_code) |
| wechat_channels_account_icon | string | 视频号头像 |
| dynamic_creative_id | integer | 创意 id |
| visibility_code | enum | 不支持沉淀的状态码，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#visibility_code) |
| jump_info | struct | 主跳转组件，文本组件 |
| component_id | integer | 创意组件 id |
| value | struct | 落地页内容结构定义，落地页组件内容结构定义,jump_info 结构单独显示,详见 [\[落地页内容结构详细定义\]](https://developers.e.qq.com/v3.0/docs/api/finder_ad_object_list/get#jump_info_struct_href) |
...
| jump_info | struct | 落地页内容结构定义，落地页组件内容结构定义,jump_info 结构单独显示,详见 [\[落地页内容结构详细定义\]](https://developers.e.qq.com/v3.0/docs/api/finder_ad_object_list/get#jump_info_struct_href) |
| page_type | enum | 落地页类型，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#page_type) |
| page_spec | struct | 落地页内容 |
| android_app_spec | struct | android 默认落地页内容，通常情况下不需要额外数据；在视频号动态落地页内部按钮跳转时，需要额外传入 android_app_id、wechat_canvas_page_id 字段。 |
| android_app_id | string | 安卓应用直达 AppId |
