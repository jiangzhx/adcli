...
| excluded_converted_audience | struct | 排除已转化人群行为定向，排除已转化用户定向范围 同应用，仅当推广产品类型为 ANDROID/IOS 应用时可以使用，没有选择自定义转化行为（excluded_dimension）时，使用该定向出价需要满足是 oCPC、oCPM 营销； 没有选择自定义转化行为（excluded_dimension）时，使用该定向出价需要满足是 oCPC、oCPM 营销； |
...
| conversion_behavior_list | enum[] | 转化行为，ENUM 值同优化目标类型，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_optimization_goal) |
...
| user_os | enum[] | 操作系统定向，该功能即将下线，仅部分行业灰度开放，如有问题可联系您的客户运营。当 marketing_carrier_type=MARKETING_CARRIER_TYPE_APP_ANDROID 时，创意只会在 Android 操作系统上展示；当 marketing_carrier_type=MARKETING_CARRIER_TYPE_APP_IOS 时，营销只会在 iOS 操作系统上展示，其他场景下均需指定 user_os。举例：当 marketing_target_type = MARKETING_TARGET_TYPE_APP_ANDROID 或 MARKETING_TARGET_TYPE_APP_IOS 时，需要传输 user_os 的值，否则会默认通投到 iOS 和 android 双端。详见 [\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_os) 当版位=腾讯平台与内容媒体电脑端，推广产品≠微信小游戏时，不支持参数 user_os。 |
...
| wechat_ad_behavior | struct | 微信营销行为定向，当且仅当投放微信营销时有效,当枚举值选择【已经添加过企业微信】需要传入企业微信 id |
| actions | enum[] | 微信再营销类型，WECHAT_OFFICIAL_ACCOUNT_FOLLOWED，关注过客户微信公众号 APP_INSTALLED，已安装你的应用（推广目标仅支持 ANDROID、IOS） WECHAT_COUPON_OBTAINED，领取过客户微信卡券 WECHAT_OFFICIAL_ACCOUNT_AD_LIKE，对微信公众号营销感兴趣 WECHAT_MOMENTS_AD_LIKE，对微信朋友圈营销感兴趣 MINI_GAME_WECHAT_REGISTERED，曾经注册过你的小游戏（仅推广目标为微信小游戏），[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_ad_behavior_route_type) |
...
| configured_status | enum | 客户设置的状态，ADX 程序化投放不可填写提交，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_configured_status) |
| created_time | integer | 创建时间，时间戳 |
| last_modified_time | integer | 最后修改时间，时间戳 |
| is_deleted | boolean | 是否已删除，true：是，false：否 |
| system_status | enum | 营销单元在系统中的状态，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_calc_ad_group_status) |
| adgroup_name | string | 营销单元，同一帐号下的营销单元不允许重复。字段长度最小 1 个等宽字符，长度最大 60 等宽字符（即字段最大长度为 60 个中文字或全角标点，120 个英文字或半角标点。一个等宽字符等价于一个中文，等价于两个英文。） |
...
| first_day_begin_time | string | 首日开始投放时间，首日开始投放时间：HH:ii:ss，ADX 程序化投放不可填写提交。 |
...
| time_series | string | 投放时间段，格式为 48 * 7 位字符串，且都为 0 和 1，以半个小时为最小粒度，从周一零点开始至周日 24 点结束。0 为不投放，1 为投放，全传 1 视为全时段投放，不允许全部传 0。朋友圈营销的投放时间需大于等于 6 小时，小于等于 30 个自然日，且每天至少投放 6 小时，且每天投放的时段需保持一致； ADX 程序化投放默认填写 336 个 1。 |
| automatic_site_enabled | boolean | 是否开启智能版位功能，建议使用。ADX 程序化投放不可填写提交。 |
...
| scene_spec | struct | 场景定向，ADX 程序化投放不可填写提交。 |
...
| union_position_package | integer[] | 定投联盟流量包列表，一个营销单元最多可选择 5 个定投流量包，且该营销单元所有流量包关联的营销位数量不得超过 10000 个 |
| exclude_union_position_package | integer[] | 屏蔽联盟流量包列表，一个营销单元最多可选择 5 个屏蔽流量包，且该营销单元所有流量包关联的营销位数量不得超过 10000 个 |
...
| display_scene | enum[] | 展示场景，当且仅当投放移动联盟流量时可以使用，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_display_scene) |
...
| mobile_union_category | integer[] | 腾讯营销联盟媒体类型场景定向，当且仅当投放腾讯营销联盟流量时（site_set = SITE_SET_MOBILE_UNION）时可以使用 |
...
| deep_conversion_spec | struct | oCPA 深度优化内容，若此字段不传，或传空则视为无限制条件，ADX 程序化投放不可填写提交。 |
...
| conversion_id | integer | 转化 id，包含优化目标，深度优化目标，数据上报方式，归因方式等信息，仅 ocpc/ocpm 营销可设置，转化 id 设置后不可修改。可通过辅助工具[\[转化查询接口\]](https://developers.e.qq.com/v3.0/docs/api/conversions/get)查询可使用的转化 id。当传入转化 id 时，可无需再传入 user_action_sets，additional_user_action_sets，optimization_goal，deep_conversion_spec 字段信息。如若同时传入转化 id 和 user_action_sets/additional_user_action_sets，将以转化 id 的信息为第一优先级；如若同时传入转化 id 和 optimization_goal/deep_conversion_spec，仅当转化 id 中包含的优化目标/深度优化目标与传入的优化目标/深度优化目标信息一致时，才允许创建，否则将不允许创建。功能灰度开放中,不支持朋友圈。ADX 程序化投放不可填写提交。 |
| deep_conversion_behavior_bid | integer | 深度优化行为的出价，使用转化 id 确认优化目标和深度优化目标的客户，深度优化行为的目标出价无需传 deep_conversion_spec 结构体中字段，可直接使用 deep_conversion_behavior_bid 字段传输深度出价。单位为分。ADX 程序化投放不可填写提交。 |
| deep_conversion_worth_rate | float | 深度优化价值的出价，使用转化 id 确认优化目标和深度优化目标的客户，深度优化价值的目标出价无需传 deep_conversion_spec 结构体中字段，可直接使用 deep_conversion_worth_rate 字段传输深度出价。比值，没有单位，ADX 程序化投放不可填写提交。 |
| deep_conversion_worth_advanced_rate | float | 强化优化价值的期望 ROI，使用转化 id 确认优化目标和强化优化目标的客户，强化优化价值的目标出价无需传 deep_conversion_spec 结构体中字段，可直接使用 deep_conversion_worth_advanced_rate 字段传输深度出价。该功能灰度开放中。ADX 程序化投放不可填写提交。 |
| deep_conversion_behavior_advanced_bid | integer | 深度辅助优化 OG 出价，使用转化 id 确认优化目标和深度辅助优化 OG 的客户，深度辅助优化 OG 出价无需传 deep_conversion_spec 结构体中字段，可直接使用 deep_conversion_behavior_advanced_bid 字段传输深度出价。单位为分。ADX 程序化投放不可填写提交。 |
| bid_mode | enum | 出价方式，ADX 程序化投放仅支持{BID_MODE_CPC,BID_MODE_CPM}，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_bid_mode) |
| auto_acquisition_enabled | boolean | 一键起量开关，一键起量开关定义。ADX 程序化投放不可填写提交。 |
| auto_acquisition_budget | integer | 一键起量预算，单位为分，一键起量预算定义。ADX 程序化投放不可填写提交。 |
| smart_bid_type | enum | 出价类型，ADX 程序化投放不可填写提交，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#smart_bid_type) |
| smart_cost_cap | integer | 自动出价下，系统计算的预计成本的上限值，单位为分，ADX 程序化投放不可填写提交。 |
...
| search_expand_targeting_switch | enum | 搜索定向拓展开关，ADX 程序化投放不可填写提交，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_search_expand_targeting_switch) |
| auto_derived_landing_page_switch | boolean | 是否开启自动衍生落地页开关，ADX 程序化投放不可填写提交。 |
| data_model_version | integer | 数据版本号 |
| bid_scene | enum | 出价场景，ADX 程序化投放不可填写提交，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#bid_scene) |
...
| deep_optimization_type | enum | 深度优化策略类型，该字段仅在深度优化开启时可用，且不可与 deep_optimization_action_type 同时使用，仅当投放微信版位营销时可设置为 DEEP_OPTIMIZATION_TYPE_SECOND_STAGE_BID。当没有指定该字段时 |
| flow_optimization_enabled | boolean | 是否使用自动流量优选，该功能已废弃，以下为原说明文档：仅支持腾讯营销联盟版位使用，且不能和腾讯营销联盟行业精选流量包(mobile_union_industry)或腾讯营销联盟媒体类型场景定向（mobile_union_category）同时使用，flow_optimization_enabled 设置为 true, 开启自动流量优选。ADX 程序化投放不可填写提交。 |
...
| search_expansion_switch | enum | 搜索扩量开关，ADX 程序化投放不可填写提交，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#search_expansion_switch) |
...
| material_package_id | integer | 素材标签 id，ADX 程序化投放不可填写提交。 |
...
| ecom_pkam_switch | enum | 一方人群跑量加强开关状态，ADX 程序化投放不可填写提交，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#ecom_pkam_switch) |
...
| auto_acquisition_status | enum | ，一键起量状态, 一键起量状态表示当前营销单元在开启一键起量时，所处的不同起量阶段。一键起量状态与一键起量开关是两个独立的字段，一键起量开关为客户创建或更新时设置的开关，正常的营销单元在一键起量过程结束后，一键起量开关不会随一键起量状态改变。起量结束后，如果想再次开启一键起量，需要关闭一键起量开关后再次开启一键起量开关。，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_auto_acquisition_status) |
...
| mpa_spec | struct | 动态商品营销属性，当营销形态为动态商品营销时，该字段必填，ADX 程序化投放不可填写提交。 |
| recommend_method_ids | integer[] | 动态创意营销的商品推荐方式，有使用权限的推荐方式 id，当营销形态为动态创意营销时，该字段必填，如填写多个推荐方式，需确保推荐方式的父节点为同一个 |
...
| dca_spec | struct | 动态内容营销属性，只有当动态营销类型 dynamic_ad_type 为动态内容营销时，才可以设置该字段 |
| recommend_method_ids | integer[] | 动态内容营销的优选方式，目前只支持设置单个优选方式，支持的优选方式可选值为：RTA 优选（取值为 95，需申请权限） |
| set_id | string | 动态内容营销的素材集合 id |
| aoi_optimization_strategy | struct | 高价值范围探索，功能灰度开放中，如需使用可联系您的客户运营。ADX 程序化投放不可填写提交。 |
...
| additional_product_spec | struct | 附加商品属性，ADX 程序化投放不可填写提交。 |
| product_catalog_id | string | 商品库 id |
| product_outer_id | string | 商品 id |
| enable_breakthrough_siteset | boolean | 是否支持版位突破，投放版位选择“微信朋友圈”，当营销有机会获得更多曝光与转化时，将可能投放到微信公众号与小程序版位。ADX 程序化投放不可填写提交。 |
| live_recommend_strategy_enabled | boolean | 直播种草人群探索，功能灰度开放中，如需使用可联系您的客户运营。ADX 程序化投放不可填写提交。 |
| custom_cost_roi_cap | float | 控制成本的期望 ROI |
| enable_steady_exploration | boolean | 是否稳步探索更多版位，当选择特定版位时 |
| adx_realtime_type | enum | ADX 程序化投放素材实时回复类型，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_adx_realtime_type) |
| smart_targeting_status | enum | 营销智能定向状态，该参数用于获取营销单元组当前的实际定向模式状态，帮助开发者确认营销组当前的定向配置方式，便于后续管理和优化。，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#smart_targeting_status) |
