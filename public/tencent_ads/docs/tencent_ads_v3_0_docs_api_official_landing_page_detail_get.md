---
title: 官方落地页-获取落地页详情 全部接口
platform: tencent_ads
source_url: https://developers.e.qq.com/v3.0/docs/api/official_landing_page_detail/get
doc_id: tencent_ads_v3_0_docs_api_official_landing_page_detail_get
source_id: tencent_ads_v3_0_docs_api_official_landing_page_detail_get
---
# 官方落地页-获取落地页详情 [全部接口](https://developers.e.qq.com/v3.0/docs/apilist)

| 所属权限 | ads_management |
| --- | --- |
| 请求地址 | official_landing_page_detail/get |
| 请求方法 | GET |
...
```
curl -v -G 'https://api.e.qq.com/v3.0/official_landing_page_detail/get?access_token=<ACCESS_TOKEN>&timestamp=<TIMESTAMP>&nonce=<NONCE>' \
-d 'page_id=12345678' \
...
| ios_app_id | string | ios 应用 id |
| android_app_id | string | android 应用 id |
| enable_android_market | boolean | 打开安卓 APP 跳转厂商功能，需要与 android_app_id 同时使用,默认为 false 默认值：false |
| share_title | string | 分享标题，不支持 emoji，需要与 share_description，share_thumburl_material_id 同时使用 |
| share_description | string | 分享描述，不支持 emoji，需要与 share_title，share_thumburl_material_id 同时使用 |
| share_thumb_url | string | 分享缩略图素材链接 |
| bg_color | string | 页面背景色 默认值：#FFFFFF |
| page_elements | struct[] | 官方落地页组件列表，支持的组件参考示例 |
...
| material_id | string | 视频素材 id，视频大小：不超过 100MB 视频尺寸：宽度 750px，高度≤ 1536px 视频长度：小于 90 秒 可通过 [\[videos 模块\]](https://developers.e.qq.com/docs/api/business_assets/video/videos_add) 上传视频后获得 |
...
| material_id | string | 图片素材 id，图片格式：PNG 或 JPG 格式 图片大小：不超过 1MB 图片尺寸：宽度大于 1px, 高度不小于 200px、不大于 1980px 通过 [\[images 模块\]](https://developers.e.qq.com/docs/api/business_assets/image/images_add) 上传图片后获得 |
...
| material_id | string | 图片素材 id，图片格式：PNG 或 JPG 格式 图片大小：不超过 1MB 图片尺寸：宽度为 4096px，高度为 2048px 通过 [\[images 模块\]](https://developers.e.qq.com/docs/api/business_assets/image/images_add) 上传图片后获得 |
...
| material_id_list | string[] | 图片素材 id 列表，每个素材的尺寸需相同 |
...
| title | struct | 标题 |
| content | string | 文案内容，不支持 emoji |
| color | string | 颜色 默认值：#000000 |
| detail | struct | 内容 |
| content | string | 文案内容，不支持 emoji |
| color | string | 颜色 默认值：#000000 |
...
| material_id | string | 图片素材 id，图片格式：PNG 或 JPG 格式 图片大小：不超过 1MB 图片尺寸：宽度为 750px，高度不大于 1336px 通过 [\[images 模块\]](https://developers.e.qq.com/docs/api/business_assets/image/images_add) 上传图片后获得 |
| areas | struct[] | 热区列表，热区数量，最少 0 个，最多 3 个；热区累积面积不得超过图片面积的 30% |
| left | number | 热区左上顶点距离图片左侧边缘的百分比，基于当前图片尺寸计算 |
| top | number | 热区左上顶点距离图片顶部的百分比，基于当前图片尺寸计算 |
| width | number | 热区宽度占图片宽度的百分比，基于当前图片尺寸计算 |
| height | number | 热区高度占图片高度的百分比，基于当前图片尺寸计算 |
...
| deeplink_url_ios | string | ios 直达链接，不填表示不开启 |
| deeplink_url_android | string | android 直达链接，不填表示不开启 |
| convert_link | struct | 转化-跳转链接配置，同一层级里仅可配置一种转化 |
| convert_type | string | 值为 link |
| src | string | url 链接，必须为 https 开头 |
| app_id_ios | string | ios 直达链接跳转的 appId，不填表示不开启 |
| deeplink_url_ios | string | ios 直达链接，不填表示不开启 |
| app_id_android | string | android 直达链接跳转的 appId，不填表示不开启 |
| deeplink_url_android | string | android 直达链接，不填表示不开启 |
| convert_weapp | struct | 转化-打开小程序配置，同一层级里仅可配置一种转化 |
| convert_type | string | 值为 weapp |
| weapp_id | string | 原始 id，gh_xxxx |
| weapp_path | string | 小程序路径 |
| backup_link | string | url 链接，必须为 https 开头 |
| convert_gh | struct | 转化-关注公众号配置，同一层级里仅可配置一种转化 |
| convert_type | string | 值为 gh |
| appid | string | wxid，以 wx 开头的字符串,如 wxffffffffffff |
| one_click | boolean | 是否开启一键关注，true: 开启, false: 关闭 默认值：false |
| direct_focus | boolean | 是否开启立即关注，true: 开启, false: 关闭。 仅图文复合系列组件可以使用，开启后，用户点击关注组件，可在当前页面快捷关注，无需进入二次确认页；开启后，组件内容使用公众号信息，「按钮文本」内容固定为【关注公众号】 默认值：false |
| convert_followVideo | struct | 转化-关注视频号配置，同一层级里仅可配置一种转化 |
| convert_type | string | 值为 followVideo |
| nick_name | string | 视频号名称 |
| one_click | boolean | 是否开启一键关注，true: 开启, false: 关闭 默认值：false |
| margin_top | number | 与上一个组件的距离 |
| margin_bottom | number | 与下一个组件的距离 |
...
| font_size | string | 字体大小 默认值：16 |
| font_weight | string | 是否加粗，normal:普通； bold:加粗 默认值：normal |
| font_style | string | 字体样式，normal:正常;italic:斜体; 默认值：normal |
...
| thickness | number | 线条粗细 默认值：1 |
| type | string | 线条类型，solid:实线;dashed:虚线; 默认值：solid |
| margin_top | number | 与上一个组件的距离 |
| margin_bottom | number | 与下一个组件的距离 |
...
| color | string | 字体颜色 默认值：#FFFFFF |
| background_color | string | 背景颜色 默认值：#296BEF |
| content | string | 按钮文案，若跳转类型为【下载 APP 】，应合规要求文案必须包含【下载】或【安装】字样，不支持 emoji |
| has_icon | boolean | 是否显示图标，true: 开启, false: 关闭 默认值：false |
...
| icon_material_id | string | 图片素材 id，图片格式：PNG 或 JPG 格式 图片大小：不超过 300KB 图片尺寸：宽度为 750px，高度为 750px 通过 [\[images 模块\]](https://developers.e.qq.com/docs/api/business_assets/image/images_add) 上传图片后获得 |
| icon_shape | string | 图片形状，rect:方头像;round:圆头像; 默认值：rect |
| title | string | 标题文案 |
| desc | string | 描述文案 |
| button_content | string | 按钮文案 |
| title_color | string | 标题颜色 默认值：#000000 |
| desc_color | string | 字体颜色 默认值：#4C4C4C |
| button_font_color | string | 按钮文案颜色 默认值：#FFFFFF |
| button_background_color | string | 按钮背景颜色 默认值：#296BEF |
| background_color | string | 组件颜色 默认值：#f0f0f0 |
| convert_unknown | struct | 转化-暂不支持，同一层级里仅可配置一种转化 |
| convert_type | string | unknown |
| convert_download | struct | 转化-下载 APP 配置，同一层级里仅可配置一种转化 |
| convert_type | string | 值为 download |
...
| title_color | string | 标题颜色 默认值：#000000 |
| desc | string | 描述文案 |
| desc_color | string | 字体颜色 默认值：#7F7F7F |
| button_content | string | 按钮文案 |
| button_font_color | string | 按钮文案颜色 默认值：#FFFFFF |
| button_background_color | string | 按钮背景颜色 默认值：#296BEF |
| card_background_color | string | 卡片背景颜色 默认值：#FFFFFF |
| card_border_color | string | 卡片边框颜色 默认值：#E5E5E5 |
| icon_material_id | string | 图片素材 id |
| margin_top | number | 与上一个组件的距离 |
| margin_bottom | number | 与下一个组件的距离 |
| convert_unknown | struct | 转化-暂不支持，同一层级里仅可配置一种转化 |
| convert_type | string | unknown |
...
| text_align | string | 文本对齐方式,支持 left,center 默认值：left |
| content | struct[] | 卡片列表 |
| title | string | 标题文案 |
| title_color | string | 标题颜色 默认值：#000000 |
| desc | string | 描述文案 |
