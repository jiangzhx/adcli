...
| dynamic_ability_type | enum | 动态投放能力类型，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#api_dynamic_ability_type) 可选值：{ DYNAMIC_TYPE_SINGLE_PRODUCT_AD, DYNAMIC_TYPE_MULTIPLE_PRODUCT_AD, DYNAMIC_TYPE_DYNAMIC_CREATIVE } |
...
| dynamic_ad_type | enum | 动态营销类型，当前仅支持设置为 DCA 动态内容营销类型，该营销单元设置后不可修改，读取时非动态营销无法读取该字段，创建创意时仅支持自定义创意的投放模式，当前功能灰度放中仅部分客户可用，如有问题可联系您的客户运营。ADX 程序化投放不可填写提交，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#dynamic_ad_type) 可选值：{ DYNAMIC_AD_TYPE_DYNAMIC_CONTENT } |
...
| adgroup_type | enum | 营销类型，[\[枚举详情\]](https://developers.e.qq.com/v3.0/docs/enums#adgroup_type) 可选值：{ ADGROUP_TYPE_SEARCH, ADGROUP_TYPE_NORMAL } |
| mpa_spec | struct | 动态商品营销属性，当营销形态为动态商品营销时，该字段必填，ADX 程序化投放不可填写提交。 |
| recommend_method_ids | integer[] | 动态创意营销的商品推荐方式，有使用权限的推荐方式 id，当营销形态为动态创意营销时，该字段必填，如填写多个推荐方式，需确保推荐方式的父节点为同一个 数组最小长度 1，最大长度 16 |
