---
title: 获取可用的推广内容资产属性 全部接口
platform: tencent_ads
source_url: https://developers.e.qq.com/v3.0/docs/api/marketing_target_asset_properties/get
doc_id: tencent_ads_v3_0_docs_api_marketing_target_asset_properties_get
source_id: tencent_ads_v3_0_docs_api_marketing_target_asset_properties_get
---
# 获取可用的推广内容资产属性 [全部接口](https://developers.e.qq.com/v3.0/docs/apilist)

| 所属权限 | ads_management,ads_insights |
| --- | --- |
| 请求地址 | marketing_target_asset_properties/get |
| 请求方法 | GET |
...
| marketing_asset_category | string | 推广内容资产类目，类目之间通过-连接 字段长度最大 128 字节 |

## 请求示例

```
curl -v -G 'https://api.e.qq.com/v3.0/marketing_target_asset_properties/get?access_token=<ACCESS_TOKEN>&timestamp=<TIMESTAMP>&nonce=<NONCE>' \
-d 'organization_id=23919277' \
...
| property_type | string | 属性值类型，ATTRIBUTE_TYPE_ENUM 枚举、ATTRIBUTE_TYPE_INPUT 自定义输入、ATTRIBUTE_TYPE_CUSTOM_PRICE 级联自定义，举例：先选有或无，选有，再输入价格、ATTRIBUTE_TYPE_CASCADE 级联枚举，举例：先选是否有值，选是，再从 children 中选择枚举值； ATTRIBUTE_TYPE_TREE 全国省市地域树形结构； ATTRIBUTE_TYPE_CATEGORY 类目树形结构； ATTRIBUTE_TYPE_SPU spuID 类型； ATTRIBUTE_TYPE_WEB_CUSTOM 自定义落地页类型； ATTRIBUTE_TYPE_RANGE 区间类型 |
