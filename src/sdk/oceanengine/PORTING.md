# OceanEngine Java SDK -> Node.js SDK Porting Guide
...
目标是把 `src/sdk/oceanengine` 收口成可发布或可被当前仓库稳定引用的 SDK。
...
- 已建立 `src/sdk/oceanengine/package.json` 和独立 `tsconfig.json`。
...
SDK 代码放在当前仓库内的 `src/sdk` 目录。不同媒体平台必须有自己的子目录：
...
src/sdk/oceanengine/
  ...
  test/
    ...
    model-generation.test.ts
```

`src/sdk/oceanengine/src/generated/*` 必须由生成器产生，不允许手工维护。
...
- 本文档路径：`src/sdk/oceanengine/PORTING.md`
...
```txt
Java SDK api/*.java   -> src/sdk/oceanengine/src/generated/apis/*.ts
Java SDK model/*.java -> src/sdk/oceanengine/src/generated/models/*.ts

src/sdk/oceanengine/src/generated/apis.ts   -> API barrel export only
src/sdk/oceanengine/src/generated/models.ts -> model barrel export only
```

生成器输入必须是本地 Java SDK 路径：

```bash
bun run src/sdk/oceanengine/scripts/phase-a-port.ts /path/to/ad_open_sdk_java
```

### 17.1 Porting Scripts

当前保留两个脚本，入口都在 `src/sdk/oceanengine/scripts/`。
...
```bash
bun run src/sdk/oceanengine/scripts/phase-a-port.ts /path/to/ad_open_sdk_java /tmp/oceanengine-phase-a
```
...
- 脚本入口：`src/sdk/oceanengine/scripts/phase-a-port.ts`
- workflow：`src/sdk/oceanengine/src/generator/workflow.ts`
- parser：`src/sdk/oceanengine/src/generator/java-parser.ts`
- emitter：`src/sdk/oceanengine/src/generator/typescript-emitter.ts`
- 测试：`src/sdk/oceanengine/test/phase-a-port.test.ts`
...
```bash
bun run src/sdk/oceanengine/scripts/generate-from-java-sdk.ts /path/to/ad_open_sdk_java
```
...
```bash
bun run src/sdk/oceanengine/scripts/generate-from-java-sdk.ts /path/to/ad_open_sdk_java /tmp/oceanengine-generated
```

全量生成后必须检查：

```bash
bun run --cwd src/sdk/oceanengine typecheck
bun run --cwd src/sdk/oceanengine test
```
...
- 输出 `src/sdk/oceanengine/src/generated/manifest.json` 或等价报告。
...
- 已实现 `phase-a-port.ts`，用于生成 Phase A 代表 API/model 和 manifest。
