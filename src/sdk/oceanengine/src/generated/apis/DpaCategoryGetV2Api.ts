...
export class DpaCategoryGetV2Api {
  ...
  async openApi2DpaCategoryGetGet(advertiserId: number, platformId: number): Promise<DpaCategoryGetV2Response> {
    const response = await this.openApi2DpaCategoryGetGetWithHttpInfo(advertiserId, platformId);
    return response.data;
  }

  async openApi2DpaCategoryGetGetWithHttpInfo(advertiserId: number, platformId: number): Promise<ApiResponse<DpaCategoryGetV2Response>> {
