...
export class FileImageAdGetV2Api {
  ...
  async openApi2FileImageAdGetGet(advertiserId: number, imageIds: string[]): Promise<FileImageAdGetV2Response> {
    const response = await this.openApi2FileImageAdGetGetWithHttpInfo(advertiserId, imageIds);
    return response.data;
  }

  async openApi2FileImageAdGetGetWithHttpInfo(advertiserId: number, imageIds: string[]): Promise<ApiResponse<FileImageAdGetV2Response>> {
    ...
    return this.apiClient.requestWithHttpInfo<FileImageAdGetV2Response>({
      ...
      queryParams: [
        ...
        { name: "image_ids", value: imageIds, collectionFormat: "csv" }
      ]
    });
  }
}
